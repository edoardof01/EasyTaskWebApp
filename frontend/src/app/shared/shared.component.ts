...
export class SharedComponent {
  ...
  getSubtaskSessions(index: number): FormArray {
    ...
    console.log(sessions.at(0).get('start'));
    return subtasks.at(index).get('subSessions') as FormArray;
  }

  getSubtaskResources(index: number): FormArray {
    const subtasks = this.taskForm.get('subtasks') as FormArray;
    let resources = subtasks.at(index).get('subResources') as FormArray
  ...
  get guidanceControl(): FormControl{
    return this.taskForm.get('userGuidance') as FormControl;
  }
  ...
  get strategyControl(): FormControl {
    return this.taskForm.get('strategies') as FormControl;
  }
